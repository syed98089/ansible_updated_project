    ...
    stages {
        stage('Checkout') {
         ...
         stage('Ansible-playbook-executing') {
            steps {
              ansiblePlaybook credentialsId: 'ansiblekey', disableHostKeyChecking: true, installation: 'ansible', inventory: '/etc/ansible/hosts', playbook: '/opt/playbook/myplaybook.yml'
               }
               }
